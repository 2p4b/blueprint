defmodule Draft.Validator.Inclusion do
    @moduledoc """
    ...
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Draft.Validator.Inclusion.validate(1, [1, 2, 3])
        {:ok, [1,2,3]}

        iex> Draft.Validator.Inclusion.validate(1, [in: [1, 2, 3]])
        {:ok, [1,2,3]}

        iex> Draft.Validator.Inclusion.validate(4, [1, 2, 3])
        {:error, "must be one of [1, 2, 3]"}

        iex> Draft.Validator.Inclusion.validate("a", ~w(a b c))
        {:ok, ~w(a b c)}

        iex> Draft.Validator.Inclusion.validate(nil, ~w(a b c))
        {:error, ~S(must be one of ["a", "b", "c"])}

        iex> Draft.Validator.Inclusion.validate(nil, [in: ~w(a b c), allow_nil: true])
        {:ok, nil}

        iex> Draft.Validator.Inclusion.validate("", [in: ~w(a b c), allow_blank: true])
        {:ok, ""}
    ...
    Custom error messages (in EEx format), provided as :message, can use the following values:
        ...
        iex> Draft.Validator.Inclusion.__validator__(:message_fields)
        [value: "The bad value", list: "List"]

    An example:

        iex> Draft.Validator.Inclusion.validate("a", in: [1, 2, 3], message: "<%= inspect value %> is not an allowed value")
