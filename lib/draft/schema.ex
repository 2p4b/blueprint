        ...
        quote do
            ...
            Enum.each(@bp_bases, fn base -> 
                ...
                |> Enum.each(fn {name, {type, opts, mod}} -> 
                    Draft.Schema.__field__(__MODULE__, name, type, opts, mod, true)
                end)
            end)

            # Create a scope to avoid leaks.
            (fn ->
                import Draft.Schema, only: [field: 2, field: 3]
            ...
            require Draft.Extract.Schema
            Draft.Extract.Schema.for_struct(@bp_rules)
            Draft.Schema.__type__(@bp_specs)

            Draft.Schema.__contructor__(@bp_keys)
    ...
    defmacro __contructor__(bp_keys) do
        quote do
            ...
            def __dump__(data, opts \\ []) do
                Draft.Type.Map.dump(data, fields: __fields__())
            end
            ...
            def __cast__(attr, _opts) when is_map(attr) do
                ...
                with {:ok, attr} <- Draft.validate_required(attr, @bp_enforce_keys) do
                    case Draft.Type.Map.cast(attr, fields: __fields__()) do
                        {:ok, data} ->
    ...
    def __field__(mod, name, type, opts, dmod, overwrite) when is_atom(name) do
        ...
        spec = 
            case Keyword.fetch(opts, :spec) do
                ...
                _ ->
                    registered_types = Draft.Registry.types()
