defmodule Draft.Validator do
    ...
    defmodule ErrorMessage do
        ...
        defp extract_error_renderer(options) do
            cond do
                ...
                renderer = Application.get_env(:draft, :error_renderer) ->
                    renderer

                true ->
                    Draft.ErrorRenderers.EEx
            end
        end
    end
    ...
    defmodule Skipping do
        ...
        def skip?(value, options) do
            cond do
                Keyword.get(options, :allow_blank) -> Draft.Blank.blank?(value)
    ...
    defmacro __using__(_) do
        quote do
            ...
            import Draft.Validator.Skipping
            use Draft.Validator.ErrorMessage
    ...
    defp do_validate_if_condition(data, {name, value}) when is_atom(name) do
        Draft.Extract.attribute(data, name) == value
    end

    defp do_validate_if_condition(data, condition) when is_atom(condition) do
        !Draft.Blank.blank?(Draft.Extract.attribute(data, condition))
    end
end
