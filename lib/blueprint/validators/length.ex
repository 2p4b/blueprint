defmodule Blueprint.Validators.Length do
    @moduledoc """
    ...
    * `:tokenizer`: A function with arity 1 used to split up a
        ...
        passed through intact. See `Blueprint.Validators.tokens/1`.
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Blueprint.Validators.Length.validate("foo", max: 2)
        {:error, "must have a length of no more than 2"}

        iex> Blueprint.Validators.Length.validate("foo", max: 2, message: "must be the right length")
    ...
    Custom error messages (in EEx format), provided as :message, can use the following values:
        ...
        iex> Blueprint.Validators.Length.__validator__(:message_fields)
        [value: "Bad value", tokens: "Tokens from value", size: "Number of tokens", min: "Minimum acceptable value", max: "Maximum acceptable value"]

    An example:

        iex> Blueprint.Validators.Length.validate("hello my darling", min: 4, tokenizer: &String.split/1,
