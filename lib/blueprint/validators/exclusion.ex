defmodule Blueprint.Validators.Exclusion do
    @moduledoc """
    ...
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Blueprint.Validators.Exclusion.validate(1, [1, 2, 3])
        {:error, "must not be one of [1, 2, 3]"}

        iex> Blueprint.Validators.Exclusion.validate(1, [in: [1, 2, 3]])
        {:error, "must not be one of [1, 2, 3]"}

        iex> Blueprint.Validators.Exclusion.validate(1, [in: [1, 2, 3], message: "<%= value %> shouldn't be in <%= inspect list %>"])
