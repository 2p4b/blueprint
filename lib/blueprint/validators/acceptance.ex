defmodule Blueprint.Validators.Acceptance do
    @moduledoc """
    ...
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Blueprint.Validators.Acceptance.validate(1, true)
        :ok

        iex> Blueprint.Validators.Acceptance.validate(nil, true)
        {:error, "must be accepted"}

        iex> Blueprint.Validators.Acceptance.validate(nil, message: "must be accepted!")
        {:error, "must be accepted!"}

        iex> Blueprint.Validators.Acceptance.validate(1, [as: "yes"])
        {:error, ~S(must be accepted with `"yes"`)}

        iex> Blueprint.Validators.Acceptance.validate("verily", [as: "verily"])
        :ok

    ## Custom Error Messages Custom error messages (in EEx format),
    ## provided as :message, can use the following values:

        iex> Blueprint.Validators.Acceptance.__validator__(:message_fields)
        [value: "The bad value"]

    An example:

        iex> Blueprint.Validators.Acceptance.validate(nil, message: "<%= inspect value %> doesn't count as accepted")
