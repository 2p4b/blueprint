defmodule Blueprint.Validator.Format do
    @moduledoc """
    ...
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Blueprint.Validator.Format.validate("foo", ~r/^f/)
        {:ok, "foo"}

        iex> Blueprint.Validator.Format.validate("foo", ~r/o{3,}/)
        {:error, "must have the correct format"}

        iex> Blueprint.Validator.Format.validate("foo", [with: ~r/^f/])
        {:ok, "foo"}

        iex> Blueprint.Validator.Format.validate("bar", [with: ~r/^f/, message: "must start with an f"])
        {:error, "must start with an f"}

        iex> Blueprint.Validator.Format.validate("", [with: ~r/^f/, allow_blank: true])
        {:ok, ""}

        iex> Blueprint.Validator.Format.validate(nil, [with: ~r/^f/, allow_nil: true])
        {:ok, nil}
    """
